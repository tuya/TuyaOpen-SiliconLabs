...
OPERATE_RET tkl_semaphore_create_init(TKL_SEM_HANDLE *handle, uint32_t sem_cnt, uint32_t sem_max)
{
    ...
    *handle = (TKL_SEM_HANDLE)osSemaphoreNew(sem_max, sem_cnt, NULL);
    if (NULL == *handle) {
 ...
 *
 * @return OPRT_OK on success.OPRT_OS_ADAPTER_SEM_WAIT_TIMEOUT means timeout. Others on error,
 * please refer to tuya_error_code.h
 */
OPERATE_RET tkl_semaphore_wait(const TKL_SEM_HANDLE handle, uint32_t timeout)
{
    ...
    osStatus_t status = osSemaphoreAcquire((osSemaphoreId_t)handle, timeout);
    if (status != osOK) {
